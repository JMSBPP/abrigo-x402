...
- [x] 01-00-PLAN.md — pnpm workspace + tsconfig + vitest + biome + analysis/uv.lock pins + .env.example + Makefile + schema-probe (Wave 0 scaffold)
- [x] 01-01-PLAN.md — STACK pin verification + protocol-spec zod loader + viem cached clients (FETCH-01)
- [x] 01-02-PLAN.md — cost-ledger JSONL append + checkBudget 90k cap + --force bypass (FETCH-02)
- [x] 01-03-PLAN.md — subgraphFreshness + blockscoutFreshness wrappers (no block_consensus per RESEARCH §H) (FETCH-03)
- [x] 01-04-PLAN.md — content-addressed cache: cacheKeyHash + manifest + deterministic payload writer (FETCH-04)
- [x] 01-05-PLAN.md — Blockscout v1 etherscan-compat client + Swap ABI decoder + dormant subgraph client + protocol-agnosticism contract test (FETCH-01 SC-5)
- [x] 01-06-PLAN.md — CLI ichi --dry-run --estimate-budget --force + cache short-circuit (FETCH-02 SC-6)
- [x] 01-07-PLAN.md — node:http x402 mock server + wrapFetchWithPayment client bridge + round-trip integration test
- [x] 01-08-PLAN.md — Wave 3 acceptance: full suite + 01-VERIFICATION-pre.md (FETCH-01..04 grid)
...
Plans:
- [x] 04.1.1-00-PLAN.md — commit pre-reg v2 supersession (retract v1 bands; AF-03 anchor)
- [x] 04.1.1-01-PLAN.md — regenerate η=0.5 fixture + scipy_canonical_ll PRIMARY estimator + revised tests
- [x] 04.1.1-02-PLAN.md — lr_test.py Option A (tick-LS null + scipy observed + LL-scale fix) + profile_likelihood.py genuine constrained-MLE CI (incl. Wave-2.5 02b orchestrator free-β rewire + 02c verdict-gate correctness fixes)
- [x] 04.1.1-03-PLAN.md — real-panel production-rep → new scipy_canonical_ll run_id `bdaf5c7ba5a2` + four-criterion gate verdict (gate_passes=FALSE 3/4, AS-OBSERVED)
- [x] 04.1.1-04-PLAN.md — hedge rerun on new run_id → 6-artifact set; DERIVED firing_condition `null_strip_unavailable`
- [x] 04.1.1-05-PLAN.md — 04-VERIFICATION append (v2) + ls_fallback_artifact_supersession_resolved:true + STATE/ROADMAP close
...
**Plans**: 4 plans in 4 waves (Wave 1: 01 baseline-maintenance commits [renderer -M fix incl. all THREE template params.->top-level points + materialize namespace + lint data/raw/<protocol> guard + ichi scrub + AF-12 REPRO-01 re-scope note + _artifacts/repro_02_baseline_sha.txt] — the standalone pre-Iteration-2 baseline-fix commit set; Wave 2: 02 pre-registered straddle rule + scripts/cost_leg_check.py + deterministic make iteration-2-full + scoped leak-check [depends_on 06-01: its leak-check uses the exact scoped-grep 06-01 writes to PRE_REGISTRATION.md, and AF-03 ordering requires 06-01 Task 3 to predate 06-02 leak-check]; Wave 3: 03 config-swap run of the frozen pipeline on steer.toml -> reports/steer_null_result.pdf [null_cost from inside the run]; Wave 4: 04 REPRO-02 empty-diff attestation [base read from repro_02_baseline_sha.txt] + steer PDF content-check + 06-VERIFICATION-pre.md [verification_pass CONDITIONAL — pending-fetch if PDF/null_cost/empty-diff unmet]). Note: the earlier 10-plan "02-*" list here was a copy-paste artifact of Phase 2 — superseded.
- [ ] 06-01-PLAN.md — baseline maintenance (pre-Iteration-2): generic renderer -P->-M fix + materialize data/raw/<protocol>/ namespace + genuine ichi-coupling scrub + AF-12 REPRO-01 scoped-grep re-scope note (REPRO-01, REPRO-02) [Backend Architect + Data Engineer + Reality Checker]
- [ ] 06-02-PLAN.md — pre-registered straddle decision rule + Q9-fallback-deferred caveat + scripts/cost_leg_check.py (verdict: FAIL -> null_cost) + make iteration-2-full + scoped leak-check + -M renderer target (REPRO-03, REPRO-04, HEDGE-05) [Reality Checker + DevOps Automator]
- [ ] 06-03-PLAN.md — config-swap run of the frozen Phase 2-5 pipeline on protocols/steer.toml -> data/raw/steer + data/fits/steer + reports/steer_null_result.pdf (null_cost from inside the completed run); zero fetch/src+analysis/src edits (REPRO-02, REPRO-04, HEDGE-05) [Data Engineer + Analytics Reporter] (checkpoint:human-verify)
- [ ] 06-04-PLAN.md — REPRO-02 empty-diff attestation + AF-03 ordering proof + verify-reproducibility steer PDF content-check + 06-VERIFICATION-pre.md acceptance grid (null AS-OBSERVED) (REPRO-01..04, HEDGE-05) [Model QA Specialist + DevOps Automator]
