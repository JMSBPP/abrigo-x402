...
- **Firing decision lives in a dedicated `analysis/src/abrigo_x402/hedge/null_result.py` module.** Single entry point. Reads `fit_report.json` + `gate_report.json` + (optionally) the per-candidate `notes/<protocol>_cost_leg_bound.md`, decides which of the **four** firing conditions tripped — (a) Phase-0 cost-leg gate fail, (b) DGP-03 LR-indistinguishable at α=0.05, (c) HEDGE-01 zero convex-dominance conditions, (d) gate passed but Carr–Madan strip is not emittable on the BIC-winner-derived characteristic function (iter-3 addition; covers MC noise floor + analytical degeneracy uniformly) — and either invokes the strip pipeline (no firing) or renders the null-result PDF (firing). Mirrors Phase 3's `orchestrator.py` single-entry pattern. The hedge CLI subcommand calls it.
...
- **One template, four context-block branches:** `reports/_templates/null_result.qmd` carries a single signature header + a switch on the firing condition that swaps in the relevant evidence block (the fourth branch — `null_strip_unavailable` — was added in iter-3 to cover the gate-passed-but-strip-not-emittable hybrid state). Same template is reused by Phase 5 for the positive-result case as a sub-template. One file to maintain.
...
> **iter-3 (2026-05-27) revision note:** A FOURTH HEDGE-05 firing condition was promoted out of `<deferred>` and into `<decisions>` above: condition (d) `null_strip_unavailable` — gate passed but Carr–Madan strip is not emittable on the BIC-winner-derived characteristic function. This covers the hybrid state where the convex-dominance gate fires (condition (c) does NOT trip) but the strip stage aborts (either because `_build_char_func_from_winner` itself raised — `char_func_source: build_failed` — or because the resulting char_func failed positivity at 2¹¹ AND 2¹²). Phase 5 PDF rendering must handle this case as a documented null with a different evidence block than (a)/(b)/(c). `null_result.py :: decide_firing_condition` returns the string `'null_strip_unavailable'`; `_evidence_branches.qmd` gets a fourth branch; 04-09 acceptance grid gets a new row (16a).
